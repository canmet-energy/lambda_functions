...
def process_file(osa_id:, osd_id:, file_id:, context:, analysis_json:)
  ...
  qaqc_col_file = osa_id + '/' + 'simulations_out.json'
  err_col_file = osa_id + '/' + 'error_col_out.json'
...
def get_s3_stream(file_id:)
  ...
  if ret_bucket.exists?
    ...
    puts "fetch_data body: #{fetch_data[:body]}"
...
datapoint_ids.each do |osd_id|
  ...
  context = []
  puts = handler(event: event, context: context)
end